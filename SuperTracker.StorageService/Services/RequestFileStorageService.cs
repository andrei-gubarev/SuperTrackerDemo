    ...
    ILogger<RequestFileStorageService> logger) : IRequestStorageService
{
    ...
    public Task StoreRequest(string? userAgent, string? referer, string? ipAddress)
    {
        ...
        lock (LockObject)
        {
            ...
            File.AppendAllText(config.GetRequestLogFilePath(), log);
        }
        return Task.CompletedTask;
    }
    
    private static string BuildLog(string? userAgent, string? referer, string ipAddress)
    {
        return $"{DateTime.UtcNow:o}|{GetValueOrNull(referer)}|{GetValueOrNull(userAgent)}|{ipAddress}{Environment.NewLine}";
    }
